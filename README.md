...
- **OME-Arrow enables queries that OME-Zarr cannot.** Because images and feature profiles are stored together in a single Arrow table, downstream tools can filter on metadata, join profile rows to pixel data, and run SQL or Arrow compute expressions without opening individual files. At plate scale this removes a whole class of file-management overhead that per-directory formats like OME-Zarr require. The storage backend - Parquet, Vortex, or LanceDB - can be swapped to match ecosystem or performance requirements without changing the data model.
